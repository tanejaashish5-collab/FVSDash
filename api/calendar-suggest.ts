...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        const { episodes, unscheduledItems, currentMonth, currentYear } = req.body;
        
        if (!Array.isArray(episodes) || !Array.isArray(unscheduledItems) || unscheduledItems.length === 0) {
            return res.status(400).json({ message: 'Valid episodes and unscheduledItems arrays are required.' });
        }
...
`;
        ...
        return res.status(500).json({ message: 'Failed to generate content suggestions.', details: error.message });
    }
}