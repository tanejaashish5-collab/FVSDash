...
import { generateEpisodeDetails, DEFAULT_EPISODES } from '@/constants';
...
export default function EpisodeDetailPage({ params }: { params: { episodeId: string } }) {
    const router = useRouter();
    const { episodeId } = params;

    // In a real app, you would fetch this by ID. Here we simulate it.
    const episode = DEFAULT_EPISODES.find(e => e.id === episodeId);
    const episodePerformance: ContentPerformanceData | undefined = episode ? {
        id: episode.id,
        title: episode.title,
        type: episode.type,
        status: episode.status,
        publishDate: '2024-08-27', // Mock data
        dueDate: episode.dueDate,
        views: 125634,
        watchTimeHours: 876,
        audienceRetention: 55,
        viewTrend: [],
    ...
    const details = generateEpisodeDetails(episodeId);
    
    if (!episodePerformance) {
        return (
            <main className="flex-1 flex items-center justify-center p-8">
                <div className="text-center">
                    <h1 className="text-2xl font-bold text-white">Episode not found</h1>
                    <button onClick={() => router.back()} className="mt-4 text-sm text-[#F1C87A] hover:underline">
                        Go Back
                    </button>
                </div>
            </main>
    ...
    return (
        <main className="flex-1 overflow-y-auto p-4 md:p-8 animate-fade-in bg-[#0B0B0B]">
             {/* Header */}
            <div className="mb-8">
                <button onClick={() => router.back()} className="flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-colors mb-4">
