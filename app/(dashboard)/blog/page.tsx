...
const BlogPage: React.FC = () => {
    ...
    const handleSavePost = async (updatedPost: BlogPost) => {
        try {
            ...
            addToast({
                ...
                message: `Changes to "${updatedPost.title}" have been saved successfully.`,
                type: 'success',
            });
        } catch (err: any) {
            addToast({
                title: 'Error',
                message: `Could not save blog post: ${err.message}`,
                type: 'error',
            });
        }
    };
    ...
    if (editingPost) {
        return <BlogEditor post={editingPost} onSave={handleSavePost} onBack={() => setEditingPost(null)} />
    }
