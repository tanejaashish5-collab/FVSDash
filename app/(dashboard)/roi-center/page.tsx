...
export default function ROICenterPage() {
    const { settings, updateIntegrationStatus } = useSettings();
    ...
    const generateNarrative = useCallback(async () => {
        ...
        setNarrative('');
        try {
            const response = await fetch('/api/roi-narrative', {
                method: 'POST',
            ...
            if (!response.ok) {
                throw new Error(data.message || 'Failed to generate AI narrative.');
            }
            ...
            if (data.narrative && data.narrative.trim() !== '') {
                setNarrative(data.narrative);
                 ...
                 addToast({
                    title: 'AI Assistant',
                    message: 'Could not generate a summary at this time. Please try again.',
                    type: 'warning',
                });
            }
