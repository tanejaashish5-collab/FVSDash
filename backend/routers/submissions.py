...
async def create_submission(
    ...
    # Dedup check: prevent duplicate submissions within 24 hours
...
async def update_submission_status(
    ...
    if client_id:
        query["clientId"] = client_id
    valid_statuses = ["Pending", "In Review", "Approved", "Scheduled", "Revision Required", "Published",
                      # Legacy compat
                      "INTAKE", "EDITING", "DESIGN", "SCHEDULED", "PUBLISHED"]
...
@router.post("/submissions/upload-media")
async def upload_submission_media(
    file: UploadFile = File(...),
    user: dict = Depends(get_current_user)
):
    """Upload a media file for a submission. Returns the file URL."""
    ...
    return {"url": result["url"], "path": result.get("path", ""), "size": len(file_data), "filename": file.filename}
