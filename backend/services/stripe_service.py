...
async def create_checkout_session(
    ...
    try:
        ...
        session = stripe_lib.checkout.Session.create(
            ...
            metadata={"user_id": user_id, "plan": plan, "plan_name": PLANS[plan]["name"]}
        )
...
async def handle_webhook_event(payload: bytes, signature: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            event_obj = stripe_lib.Webhook.construct_event(payload, signature, webhook_secret)
        except stripe_lib.error.SignatureVerificationError as e:
            return {"success": False, "error": f"Invalid signature: {str(e)}"}

        # Normalise to a simple dict-like structure for the handler below
        class _Event:
            def __init__(self, e):
