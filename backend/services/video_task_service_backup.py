...
# =============================================================================
# LTX INTEGRATION (LOCAL + API)
# =============================================================================

async def create_ltx_job(task_data: dict) -> VideoJobResult:
    """
    Create a video generation job with LTX (local or API).

    Tries in order:
    1. LTX Desktop (local) if running on localhost:8080
    2. LTX API via fal.ai if FAL_KEY is set
    3. Falls back to mock if neither available
    """
    import aiohttp
    import asyncio

    # Get configuration
    use_local = os.environ.get("USE_LTX_LOCAL", "true").lower() == "true"
    ltx_local_url = os.environ.get("LTX_LOCAL_URL", "http://localhost:8080")
    ...
    job_id = f"ltx-{uuid.uuid4().hex[:12]}"

    # Try local LTX Desktop first
    if use_local:
        try:
            # Health check
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{ltx_local_url}/health", timeout=2) as resp:
                    if resp.status == 200:
                        logger.info("LTX Desktop is running, using local generation")
                        ...
                        if script_text and len(script_text) > 20:
                            ...
                            if aspect_ratio == "9:16":
                                result = await produce_short(
                                    ...
                                    voice_id=None,
                                    job_id=job_id,
                                    use_ltx_local=True  # Flag to use LTX instead of Kling
                                )
                            else:
                            ...
                            logger.info(f"LTX local full pipeline video: {video_url}")
                        else:
                            # Single clip generation
                            enhanced_prompt = enforce_chanakya_aesthetic(raw_prompt)

                            # Call LTX Desktop API
                            async with session.post(
                                f"{ltx_local_url}/generate",
                                json={
                                ...
                                if resp.status == 200:
                                    ...
                                    logger.info(f"LTX local clip generated: {video_url}")
            ...
            logger.warning(f"LTX Desktop not available or failed: {e}")

    # Fall back to LTX API via fal.ai
    if fal_key:
        try:
            import fal_client

            logger.info("Using LTX API via fal.ai")

            # Enhanced prompt
            enhanced_prompt = enforce_chanakya_aesthetic(raw_prompt if not script_text else script_text[:200])

            # Submit to LTX API (cheaper than Kling)
            result = await fal_client.subscribe_async(
                "fal-ai/ltx-2/text-to-video/fast",  # Fast model at $0.04/second
                arguments={
                    "prompt": enhanced_prompt,
                    "duration": 10,
                    "aspect_ratio": aspect_ratio.replace(":", "_"),  # "16_9" format
                }
            )
            ...
            while result.status != "completed":
                ...
                result = await fal_client.status_async(result.request_id)

            video_url = result.data.get("video_url")
            logger.info(f"LTX API video generated: {video_url}")
            ...
            logger.error(f"LTX API generation failed: {e}")

    # Fall back to mock
    logger.warning("Neither LTX Desktop nor API available, using mock")
    mock_job_id = f"ltx-mock-{int(_time.time())}-{uuid.uuid4().hex[:8]}"
    return VideoJobResult(
        job_id=mock_job_id,
        provider="mock_ltx",
        is_mocked=True,
        warning="LTX not configured. Start LTX Desktop or set FAL_KEY."
    )


async def check_ltx_job(job_id: str) -> VideoStatusResult:
    """
    Check status of an LTX video job.

    Since LTX generates synchronously (both local and API), jobs are immediately READY.
    """
    # Mock jobs
    if job_id.startswith("ltx-mock"):
        parts = job_id.split("-")
        try:
            created_ts = int(parts[2])
            if _time.time() - created_ts >= 30:
                ...
                return VideoStatusResult(
                    ...
                    video_url=mock_url,
                    is_mocked=True,
                    warning="Using mock video (LTX not configured)"
                )
        except (ValueError, IndexError):
            pass
        return VideoStatusResult(status="PROCESSING", is_mocked=True)

    # Real LTX jobs are already READY (synchronous generation)
    return VideoStatusResult(
        status="READY",
        is_mocked=False,
...
async def create_video_job(provider: str, task_data: dict) -> VideoJobResult:
    ...
    Supports:
    - ltx: LTX Desktop (local) or LTX API (fal.ai) - RECOMMENDED
    - veo: Google Veo (REAL with VEO_API_KEY, or mocked fallback)
    - kling: Kling via fal.ai (REAL with FAL_KEY)
    - runway: MOCKED
    ...
    """
    if provider == "ltx":
        # LTX is the preferred provider - local (free) or API (cheaper than Kling)
        return await create_ltx_job(task_data)
    elif provider == "veo":
...
async def check_video_job(provider: str, job_id: str) -> VideoStatusResult:
    ...
    """
    if provider == "ltx":
        return await check_ltx_job(job_id)
    elif provider == "veo":
...
async def create_video_task(client_id: str, data: dict) -> dict:
    ...
    """
    valid_providers = ["ltx", "runway", "veo", "kling"]
