...
export default function BlogPage() {
  ...
  const handleGenerateBlog = async () => {
    if (!generateTopic.trim()) { toast.error('Please enter a topic'); return; }
    setGenerating(true);
    try {
      const res = await axios.post(`${API}/blog/generate`, { topic: generateTopic }, { headers: authHeaders });
      toast.success('Blog post generated!');
      setShowGenerateModal(false);
      setGenerateTopic('');
      fetchData();
    } catch (err) {
      toast.error(err.response?.data?.detail || 'Failed to generate blog post');
    } finally {
      setGenerating(false);
    }
  };
  ...
  return (
    <div data-testid="blog-page" className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <Button onClick={() => setShowGenerateModal(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white">
          Generate Blog Post
        </Button>
      </div>
      ...
      {showGenerateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 w-full max-w-md shadow-2xl">
            <h2 className="text-lg font-semibold text-white mb-4">Generate Blog Post</h2>
            <Input
              placeholder="Enter a topic (e.g. 'How to grow on YouTube Shorts')"
              value={generateTopic}
              onChange={(e) => setGenerateTopic(e.target.value)}
              className="mb-4 bg-zinc-950 border-zinc-800 text-white"
              onKeyDown={(e) => e.key === 'Enter' && handleGenerateBlog()}
            />
            <div className="flex gap-2 justify-end">
              <Button variant="ghost" onClick={() => { setShowGenerateModal(false); setGenerateTopic(''); }} className="text-zinc-400">
                Cancel
              </Button>
              <Button onClick={handleGenerateBlog} disabled={generating} className="bg-indigo-600 hover:bg-indigo-700 text-white">
