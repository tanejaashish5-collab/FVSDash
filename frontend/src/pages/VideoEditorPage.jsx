...
function SortableClip({ clip, isSelected, onSelect, onRemove }) {
  ...
  return (
    <div ref={setNodeRef} style={style}
      ...
      {/* Mini trim indicator bar */}
      {clip.duration > 0 && (
        <div className="mx-1.5 mb-1.5 h-1.5 bg-zinc-900 rounded-sm overflow-hidden" title="Trim region">
          <div className="h-full bg-violet-500/40 rounded-sm"
            style={{
              marginLeft: `${((clip.trimStart || 0) / clip.duration) * 100}%`,
              marginRight: `${(1 - ((clip.trimEnd ?? clip.duration) / clip.duration)) * 100}%`,
            }} />
        </div>
      )}
...
export default function VideoEditorPage() {
  ...
  const handleDeleteProject = async (projectId) => {
    ...
    try {
      await axios.delete(`${API}/video-editor/projects/${projectId}`, { headers: authHeaders });
      await loadProjects();
        ...
        <div className="w-56 shrink-0 border-r border-zinc-800 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto py-1">
            {projects.filter(p => !projectSearch || p.title?.toLowerCase().includes(projectSearch.toLowerCase())).map(p => (
              <div key={p.id} className={`group flex items-center px-3 py-2 text-xs transition-colors cursor-pointer ${
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 text-zinc-700 hover:text-red-400 transition-all ml-1 shrink-0 p-0.5"
            ...
            <div className="flex-1 flex flex-col min-w-0 border-r border-zinc-800">
              ...
              {selectedClip && (
                <div className="shrink-0 bg-zinc-950 border-t border-zinc-800 p-3 space-y-2">
                  ...
                  {/* Draggable trim bar — CapCut-style */}
                  {previewDuration > 0 && (
                    <div className="relative h-6 bg-zinc-900 rounded-md overflow-hidden select-none"
                      ref={el => { if (el) el.dataset.trimBar = 'true'; }}>
                      {/* Inactive region (before trim start) */}
                      <div className="absolute top-0 bottom-0 bg-zinc-950/80"
                        style={{ left: 0, width: `${((selectedClip.trimStart || 0) / previewDuration) * 100}%` }} />
                      {/* Inactive region (after trim end) */}
                      <div className="absolute top-0 bottom-0 bg-zinc-950/80"
                        style={{ left: `${((selectedClip.trimEnd ?? previewDuration) / previewDuration) * 100}%`, right: 0 }} />
                      {/* Active trim region */}
                      <div className="absolute top-0 bottom-0 bg-violet-500/20 border-y border-violet-500/40"
                        style={{
                          left: `${((selectedClip.trimStart || 0) / previewDuration) * 100}%`,
                          right: `${(1 - ((selectedClip.trimEnd ?? previewDuration) / previewDuration)) * 100}%`,
                        }}>
                        {/* Playhead position indicator */}
                        {previewTime >= (selectedClip.trimStart || 0) && previewTime <= (selectedClip.trimEnd ?? previewDuration) && (
                          <div className="absolute top-0 bottom-0 w-0.5 bg-white/60 pointer-events-none"
                            style={{ left: `${((previewTime - (selectedClip.trimStart || 0)) / ((selectedClip.trimEnd ?? previewDuration) - (selectedClip.trimStart || 0))) * 100}%` }} />
                        )}
                      </div>
                      {/* Left handle (trim start) */}
                      <div
                        className="absolute top-0 bottom-0 w-2 cursor-col-resize z-10 group/handle"
                        style={{ left: `calc(${((selectedClip.trimStart || 0) / previewDuration) * 100}% - 4px)` }}
                        onMouseDown={e => {
                          e.preventDefault(); e.stopPropagation();
                          const bar = e.target.closest('[data-trim-bar]');
                          if (!bar) return;
                          const rect = bar.getBoundingClientRect();
                          const onMove = (me) => {
                            const x = Math.max(0, Math.min(me.clientX - rect.left, rect.width));
                            const t = parseFloat(((x / rect.width) * previewDuration).toFixed(2));
                            const maxStart = (selectedClip.trimEnd ?? previewDuration) - 0.1;
                            updateClip(selectedClip.id, { trimStart: Math.min(t, maxStart) });
                          };
                      ...
                      >
                        <div className="h-full w-1 mx-auto bg-emerald-400 rounded-full group-hover/handle:bg-emerald-300 group-hover/handle:w-1.5 transition-all" />
                      </div>
                      {/* Right handle (trim end) */}
                      <div
                        className="absolute top-0 bottom-0 w-2 cursor-col-resize z-10 group/handle"
                        style={{ left: `calc(${((selectedClip.trimEnd ?? previewDuration) / previewDuration) * 100}% - 4px)` }}
                        onMouseDown={e => {
                          e.preventDefault(); e.stopPropagation();
                          const bar = e.target.closest('[data-trim-bar]');
                          if (!bar) return;
                          ...
                          const onMove = (me) => {
                            const x = Math.max(0, Math.min(me.clientX - rect.left, rect.width));
                            const t = parseFloat(((x / rect.width) * previewDuration).toFixed(2));
                            const minEnd = (selectedClip.trimStart || 0) + 0.1;
                            updateClip(selectedClip.id, { trimEnd: Math.max(t, minEnd) });
                          };
                          const onUp = () => { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); };
                          document.addEventListener('mousemove', onMove);
                          document.addEventListener('mouseup', onUp);
                      ...
                      >
                        <div className="h-full w-1 mx-auto bg-red-400 rounded-full group-hover/handle:bg-red-300 group-hover/handle:w-1.5 transition-all" />
                      </div>
                      {/* Time labels */}
                      <div className="absolute left-1 top-0.5 text-[8px] text-emerald-400/70 pointer-events-none font-mono">
                        {fmtTime(selectedClip.trimStart || 0)}
                      </div>
                      <div className="absolute right-1 top-0.5 text-[8px] text-red-400/70 pointer-events-none font-mono">
                        {fmtTime(selectedClip.trimEnd ?? previewDuration)}
                      </div>
                    </div>
                  )}

                  {/* Set In / Set Out (precise fallback) */}
                  <div className="flex items-center gap-2">
